...
opcoes_sair = ["1","2","3"]
#posicao_nova = []
equipe = ["Equipe1","Equipe2","Equipe3","Equipe4","Equipe5","Equipe6","Equipe7","Equipe8","Equipe9","Equipe10","Equipe11",]
posiçao = []
posicoes_usadas = []
...
while True:
    ...
    if escolha == "1":
        for i in range(len(equipe)):
            print(f"O valor das NTFs vinculadas à {equipe[i]} é de R${valor[i]}")
        sair = sair_continuar()
        if sair:
            continue
        else:
    ...
    if escolha == "2":
        for i in range(len(equipe)):
            position = int(posicao(equipe, i, posicoes_usadas))
            posiçao.append(position)
            print(f"Posição da {equipe[i]}: {position}")       
       
        novos_valores = calculo_ntf(valor, posiçao)
        
        for i in range(len(equipe)):
            print(f"Equipe: {equipe[i]}")
